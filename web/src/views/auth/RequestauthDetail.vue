  ...
  <section class="page">
    <header class="page-header detail-header detail-header--compact">
      <div class="detail-header-row">
        <button class="secondary-button detail-back-button" type="button" @click="goBack">
          ← {{ $t('Form.BackToAPIAuthentication') }}
        </button>
    ...
    <div class="panel">
      <div v-if="!editMode" class="detail-tab-toolbar">
        ...
        <button class="primary-button detail-edit-button" data-testid="requestauth-edit-open" type="button" @click="openEditMode">
          {{ $t('Form.Edit') }}
        </button>
      </div>
...
import BasicSetting from '../../components/auth/EditSetting.vue';

export default {
  name: 'AuthRequestDetail',
  components: {
    BasicSetting,
  },
  ...
  methods: {
    fetchData: async function() {
      ...
      await this.$store.dispatch('AuthRequest_GetItem', {
        name: this.name,
